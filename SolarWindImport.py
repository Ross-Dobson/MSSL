...
def import_omni_month(StartDateTime, EndDateTime, Resolution='1min',
    ...
    # Make DateTime the index of the dataframe - ie the row "headers"
...
def main():
    ...
    data = pd.DataFrame(columns=['Year', 'Day', 'Hour', 'Minute', 'B_IMF_ScID',
                                 'Plasma_ScID', 'IMFAvPoints',
                                 'PlasmaAvPoints', 'PercentInterp',
                                 'Timeshift', 'RMSTimeshift',
                                 'RMSPhaseFrontNormal', 'TimeBetweenObs', 'B',
                                 'B_X_GSM', 'B_Y_GSE', 'B_Z_GSE',
                                 'B_Y_GSM', 'B_Z_GSM', 'RMSBScalar',
                                 'RMSFieldVector', 'V', 'V_X_GSE', 'V_Y_GSE',
                                 'V_Z_GSE', 'n_p', 'T', 'P', 'E', 'Beta',
                                 'AlfvenMachff', 'X_SC_GSE', 'Y_SC_GSE',
                                 'Z_SC_GSE', 'X_BSN_GSE', 'Y_BSN_GSE',
                                 'Z_BSN_GSE', 'AE', 'AL', 'AU', 'SymD', 'SymH',
                                 'AsyD', 'AsyH', 'PCN', 'MagnetosonicMach',
                                 '10MeVProton', '30MeVProton', '60MeVProton'])
    ...
    for i in range(0, 12):
        ...
        endDT.append(datetime.datetime(year, (i+1), month_end_dates[i],
                                       23, 59, 59))
        this_month_data = import_omni_month(startDT[i], endDT[i], Resolution='1min', Columns='All')
